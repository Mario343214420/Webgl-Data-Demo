...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      list.forEach((item) => {
        fbxLoader.load(`http://192.168.1.66:8000/${item}/${item}.fbx`, fbx => {
          this.mapGroup.add(fbx)
        })
      }, progress => {
      ...
      fbxLoader.load('./models/fangpenqi(1).FBX', fbx => {
        fbx.castShadow = true
        ...
        this.baseLines.add(this.renderLine(['单闸板防喷器'], [26, 0, 0], [0, 0, 0]))
        this.baseLines.add(this.renderLine(['双闸板防喷器'], [26, -10, 0], [0, -5, 0]))
        this.baseLines.add(this.renderLine(['环形防喷器'], [26, 15, 0], [0, 10, 0]))
        this.scene.add(this.baseLines)
        this.group.scale.set(0, 0, 0)
        this.baseLines.scale.set(0, 0, 0)
      }, progress => {
        console.log(progress)
      }, err => {
        console.log(err)
      })
      this.mapGroup.rotation.x = this.towerGroup.rotation.x = -Math.PI / 2
      this.mapGroup.position.y = this.towerGroup.position.y = 50
    ...
    onDocumentMouseDown(e) {
      ...
      this.mX = (e.offsetX / this.$refs.cvs.width) * 2 - 1
      this.mY = -(e.offsetY / this.$refs.cvs.height) * 2 + 1
      this.floatPos = {
        x: e.offsetX,
        y: e.offsetY
      }
      ...
      const intersects = raycaster.intersectObjects([...this.group.children[0].children, ...this.towerGroup.children])
      if (intersects[0] && intersects[0].object.name) {
        this.flag = true
        let focus = intersects[0].point
        // console.log(intersects[0])
        if (intersects[0].object.parent.name === 'tower_1' || intersects[0].object.parent.name === 'tower_2') {
          console.log(intersects[0])
          this.group.scale.set(0.5, 0.5, 0.5)
          this.baseLines.scale.set(1, 1, 1)
          this.mapGroup.visible = false
          this.towerGroup.scale.set(0, 0, 0)
        } else {
