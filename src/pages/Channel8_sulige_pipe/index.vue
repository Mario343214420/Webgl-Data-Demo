<template>
  <div class="channel" ref="channel">
    <div class="mark-group">
      ...
      <div class="row current-model-control">
        <span @click="showLineGroupInfo">显示管线</span>
        <span @click="toggleBaseModelShow">切换基本模型</span>
      </div>
    </div>
  ...
  methods: {
    ...
    calculateCoordinate(baseArr) {
      ...
      baseArr.forEach(item => {
        resultArr.push([(item[0] - 108.59) * 89000, item[1] * 89000, (item[2] - 38.22) * 89000])
      })
      return resultArr
    },
    // 生成粗线体
    ...
    init() {
      ...
      let stackList = [
        [-244.29101898082408, 4.403327563306448, 92.69455327598116],
        [-176.22580257869507, 6.222537470372345, 68.67977509885671],
        [-76.5277844479517, 4.350637709413604, 13.177798832142306],
        [27.130963615084614, 5.5878192085283445, -47.46644557150306],
        [262.1845324376154, 3.5090008191589277, -193.71835271554255]
      ]
      let stackListXYZ = [
        {
          x: -244.29101898082408,
          y: 4.403327563306448,
          z: 92.69455327598116
        },
        {
          x: -176.22580257869507,
          y: 6.222537470372345,
          z: 68.67977509885671
        },
        {
          x: -76.5277844479517,
          y: 4.350637709413604,
          z: 13.177798832142306
        },
        {
          x: 27.130963615084614,
          y: 5.5878192085283445,
          z: -47.46644557150306
        },
        {
          x: 262.1845324376154,
          y: 3.5090008191589277,
          z: -193.71835271554255
        }
      ]
      let realStackList = [
        [108.59238, 0, 38.22184],
        [108.59314, 0, 38.22207],
        [108.59426, 0, 38.22256],
        [108.59544, 0, 38.22312],
        [108.59757, 0, 38.22419]
      ]

      // 缩放计算、夹角计算
      /* let realScaleCoefficientList = [0,0,0,0]
      let angle = 0
      realScaleCoefficientList.forEach((item, index) => {
        // realStackList[index]
        console.log('桩连线序号', index)
        let realLen = Math.sqrt(Math.pow(realStackList[index][0] - realStackList[index+1][0] , 2) + Math.pow(realStackList[index+1][1] - realStackList[index+1][1], 2))
        console.log('realLen:', realLen)
        let modelLen = Math.sqrt(Math.pow(stackList[index][0] - stackList[index+1][0] , 2) + Math.pow(stackList[index+1][2] - stackList[index+1][2], 2))
        console.log('modelLen:', modelLen)
        console.log('缩放比例:', modelLen/realLen)
        // let k = (stackList[index+1][1] - stackList[index][1])/stackList[index+1][0] - stackList[index][0]
        let realK = (realStackList[index+1][1]-realStackList[index][1])/(realStackList[index+1][0]-realStackList[index][0])
        let modelK = (stackList[index+1][2]-stackList[index][2])/(stackList[index+1][0]-stackList[index][0])
        console.log('连线斜率:', realK, modelK)
        console.log('求直线夹角:', 'tanα=(K1-K2)/(1+K1*K2)', Math.atan((realK-modelK)/(1+realK*modelK)))
        if(index === 1) {
          angle = Math.atan((realK-modelK)/(1+realK*modelK))
        }
      }) */

      /* stackList.forEach(item => {
        let sphereGeo = new THREE.SphereGeometry(1, 6, 4)
      ...
      pipeList.forEach(item => {
        points.push(new THREE.Vector3((item[0]-108.59)*88700, (item[1]-38.22)*88700, 0))
      })
      ...
      const realLineMaterial = new THREE.LineBasicMaterial({
        // vertexColors: true,
        color: 0x00ffff
      })
      const realPoints = []
      realStackList.forEach(item => {
        realPoints.push(new THREE.Vector3((item[0]-108.59)*88700, (item[1]-38.22)*88700, 0))
      })
      const realLineGeometry = new THREE.BufferGeometry().setFromPoints( realPoints );
      const modelLine = new THREE.Line(lineGeometry, lineMaterial)
      const realLine = new THREE.Line(realLineGeometry, realLineMaterial) */
      // line2.position.set(realPoints[0].x, ,0)
      ...
      let modelLine = this.creatFatLine(this.calculateCoordinate(pipeList), 0x00ff00, 4)
      let realLine = this.creatFatLine(this.calculateCoordinate(realStackList), 0xff0000, 4)
      // ↑↑↑↑↑ 线结构体压缩方法 ↑↑↑↑↑ //

      // modelLine.rotation.x = -Math.PI/2
      // realLine.rotation.x = -Math.PI/2
