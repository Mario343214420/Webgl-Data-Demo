...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      modelPipePoints.forEach(item => {
        const geometry = new THREE.BufferGeometry().setFromPoints([
          new THREE.Vector3( item.x, item.y + 20, item.z ),
          new THREE.Vector3( item.x, item.y - 20, item.z )
        ]);
        const material = new THREE.LineBasicMaterial({
          color: 0xff1111
      ...
      /*
      let repairPoint0913 = new THREE.CatmullRomCurve3(landCrossPoints0913.map(item => new THREE.Vector3(item[0], item[1], item[2]))).getPoints(500)
      // console.log(repairPoint0913)
      repairPoint0913.forEach((item, index) => {
        console.log('x差值: ',item.x - modelPipePoints[index].x)
        console.log('y差值: ',item.y - modelPipePoints[index].y)
        console.log('z差值: ',item.z - modelPipePoints[index].z)
        this.pipeDeepList[index] = [item.x, item.y - modelPipePoints[index].y, item.z]
      }) */
      ...
      // console.log(landCrossPoints0909)
      ...
      // changeRangeBigger20perList.forEach((item, index) => {
      //   let clone = srb.clone()
      //   clone.position.set(item[0] - baseGPSParam[1], landCrossPoints0909[indexList[index]].y, item[2] - baseGPSParam[0])
