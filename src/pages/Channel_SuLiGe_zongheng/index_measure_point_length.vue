...
export default {
  ...
  computed: {
    ...
    realToModelParam: function () {
      return 3.13 / Math.sqrt(
        Math.pow(-196.76275599897488 + 196.6977234715932,2) +
        Math.pow(-3.3393170480783283 + 4.2678474276007705,2) +
        Math.pow(-76.60252774247772 + 76.52697771441012,2)
      )
    }
  },
  mounted() {
  ...
  methods: {
    ...
    init() {
      ...
      let realLine = this.creatFatLine(this.calculateCoordinate(realStackList), 0x00ff00, 4)
      // this.pipeGroup.add(modelLine)
      realLine.name = '桩位连线'
      this.pipeGroup.name = '管线组'
      this.pipeGroup.add(realLine)
      this.pipeGroup.position.set(-258.29577, -13.82674, -116.35270)
      this.pipeGroup.rotation.set(-2.79089, 0.92459, -0.23775)
      this.pipeGroup.scale.set(0.28590, 0.28590, 0.28590)
