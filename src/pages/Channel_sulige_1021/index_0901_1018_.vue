...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.compareGroup.rotation.set(-2.9351413820098977, -1.545857762142114, -2.932936683889785)
      ...
      // 地形变化标记组
      const changeMarkGroup = new THREE.Group()
      const cMarkGeo = new THREE.SphereGeometry( 0.4, 8, 8 );
      const cMarkMtl = new THREE.MeshBasicMaterial( { color: 0x0fff0f } );
      const cMarkSphere = new THREE.Mesh( cMarkGeo, cMarkMtl );
      let changeArea = [
        [-120,	-3.627,	39]	,
        [-290,	-6.558,	47]	,
        [-299,	-7.024,	47]	,
      ...
      changeArea.forEach(item => {
        const copyMarkBall = cMarkSphere.clone()
        copyMarkBall.position.set(item[0], item[1], item[2])
        changeMarkGroup.add(copyMarkBall)
      })

      this.scene.add(changeMarkGroup)
